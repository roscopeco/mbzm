...
// Result codes
#define ERROR_MASK        0xff00        /* Mask used to determine if result is an error     */
#define OK                0x0000        /* Return code for "all is well"                    */
#define BAD_DIGIT         0x0100        /* Bad digit when converting from hex               */
#define GOT_EOF           0x0200        /* Got EOF when reading from stream                 */
#define BAD_HEADER_TYPE   0x0300        /* Bad header type in stream (probably noise)       */
#define BAD_FRAME_TYPE    0x0400        /* Bad frame type in stream (probably noise)        */
#define CORRUPTED         0x0500        /* Corruption detected in header (probably noise)   */
#define BAD_CRC           0x0600        /* Header did not match CRC (probably noise)        */
#define OUT_OF_RANGE      0x0700        /* Conversion attempted for out-of-range number     */
#define OUT_OF_SPACE      0x0800        /* Supplied buffer is not big enough                */
#define UNSUPPORTED       0xff00        /* Attempted to use an unsupported protocol feature */
